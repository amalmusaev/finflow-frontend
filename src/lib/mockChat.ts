...
/**
 * Генерирует реалистичный и структурированный ответ финансового ИИ-агента на основе вопроса, изображений и текущих данных.
 */
export async function generateMockAiResponse(
  ...
  const hasImages = Boolean(context?.hasImages || (context?.imagesCount && context.imagesCount > 0));
  const imagesCount = context?.imagesCount || (hasImages ? 1 : 0);
  ...
  // 0. Обработка загруженных изображений (Vision / OCR / Документы)
  if (hasImages) {
    const defaultAccount = accounts[0]?.name || 'Основной счет (Карта)';
    const groceryCategory = categories.find(c => c.name.toLowerCase().includes('продукт') || c.name.toLowerCase().includes('супермаркет'))?.name || 'Продукты';

    // А) Распознавание чека / квитанции
    if (
      query.includes('чек') ||
      query.includes('квитанц') ||
      query.includes('купил') ||
      query.includes('расход') ||
      query.includes('скан') ||
      query.includes('распознай') ||
      query === '' ||
      query.includes('чек на')
    ) {
      return `### 🧾 Распознан финансовый документ (Чек / Квитанция)

Я проанализировал прикрепленн${imagesCount > 1 ? `ые ${imagesCount} изображения` : 'ое изображение'} и извлек данные кассового чека:

* **Организация:** \`ООО «Супермаркет-Ритейл»\` (ИНН 7701234567)
* **Дата и время:** \`${new Date().toLocaleDateString('ru-RU')} 14:32\`
* **Тип операции:** Расход наличными / картой
* **Предлагаемый счет списания:** **${defaultAccount}**
* **Категория:** **${groceryCategory}**

#### 📋 Позиции в чеке:

| Наименование | Кол-во | Цена | Итого |
| :--- | :--- | :--- | :--- |
| **Кофе зерновой Arabica 250г** | 1 шт. | \`480.00 ₽\` | \`480.00 ₽\` |
| **Молоко ультрапастеризованное 3.2%** | 2 шт. | \`115.00 ₽\` | \`230.00 ₽\` |
| **Хлеб ремесленный цельнозерновой** | 1 шт. | \`95.00 ₽\` | \`95.00 ₽\` |
| **Сыр выдержанный Gouda 200г** | 1 шт. | \`340.00 ₽\` | \`340.00 ₽\` |
| **Фруктовый набор (Яблоки, Бананы)** | 1.4 кг | \`180.00 ₽/кг\` | \`252.00 ₽\` |

---
* **Сумма без скидки:** \`1 520.00 ₽\`
* **Скидка по карте лояльности:** \`-123.00 ₽\`
* **ИТОГО К ОПЛАТЕ:** \`1 397.00 ₽\`
* **В т.ч. НДС 20%:** \`232.83 ₽\`

> 💡 **Рекомендация Finflow:** Вы можете перейти в раздел **«Операции»** и добавить операцию расхода на сумму **1 397 ₽** в категорию **«${groceryCategory}»**.`;
    }

    // Б) Распознавание графика / диаграммы
    if (query.includes('график') || query.includes('диаграмм') || query.includes('динамик') || query.includes('тренд')) {
      return `### 📈 Анализ финансового графика / диаграммы

Я распознал структуру графических данных на прикрепленн${imagesCount > 1 ? `ых ${imagesCount} изображениях` : 'ом изображении'}:

* **Тип визуализации:** Динамика финансовых показателей (Временной ряд)
* **Основной тренд:** Умеренный восходящий рост доходности (+14.2% к предыдущему кварталу).
* **Пиковая точка:** Зафиксирована в середине расчетного периода.
* **Волатильность:** В пределах нормы (стандартное отклонение не превышает 6.5%).

#### 📊 Ключевые выводы:
1. Текущие накопления опережают инфляционный коридор.
2. Рекомендуется зафиксировать часть положительного остатка на накопительном вкладе.`;
    }

    // В) Общий интеллектуальный анализ изображения
    return `### 🖼️ Анализ прикрепленного изображения

Я успешно обработал файл${imagesCount > 1 ? `ы (${imagesCount} шт.)` : ''}${context?.imageNames?.length ? `: \`${context.imageNames.join(', ')}\`` : ''}.

* **Тип данных:** Финансовый документ / скриншот
* **Обнаруженные элементы:** Числовые значения сумм, даты транзакций, текстовые метки контрагентов.
* **Контекст запроса:** *«${userQuery || 'Анализ прикрепленного файла'}»*

#### 📌 Распознанные параметры:
* **Основная валюта:** \`RUB (₽)\`
* **Совпадение со счетами:** Найдено соответствие с вашим портфелем Finflow (**${accounts.length}** счетов).

> 💡 *Если на изображении чек или банковская выписка, вы можете спросить: «Разбери позиции чека» или «Добавь эти траты в операции».*`;
  }
